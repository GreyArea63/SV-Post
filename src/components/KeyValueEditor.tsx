...
// ИСПРАВЛЕНИЕ 3.41: обёртка через useCallback + useRef для стабильных ссылок
export const KeyValueEditor = memo<KeyValueEditorProps>(({
  ...
  // Используем useRef для хранения последних версий функций
  ...
  const handleCheckboxChange = useCallback((id: string, checked: boolean) => {
    onUpdateRef.current(field, id, 'enabled', checked);
  }, [field]);

  const handleKeyChange = useCallback((id: string, value: string) => {
    onUpdateRef.current(field, id, 'key', value);
  }, [field]);

  const handleValueChange = useCallback((id: string, value: string) => {
    onUpdateRef.current(field, id, 'value', value);
  }, [field]);

  const handleRemove = useCallback((id: string) => {
    onRemoveRef.current(field, id);
  }, [field]);

  const handleAdd = useCallback(() => {
    onAddRef.current(field);
  }, [field]);
