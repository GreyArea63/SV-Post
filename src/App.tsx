...
const ToastContainer = memo(({ toasts, onRemove }: { toasts: Toast[]; onRemove: (id: string) => void }) => (
  <div className="fixed top-16 right-4 z-[300] space-y-2">
    {toasts.map(toast => (
      <div
        key={toast.id}
        className={`toast flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg min-w-[300px] animate-fade-in ${
    ...
    <div className="bg-[#1e1e1e] border border-[rgba(255,255,255,0.08)] rounded-lg shadow-2xl w-full max-w-md p-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">IndexedDB повреждена</p>
        </div>
      </div>
      <p className="text-sm text-gray-300 mb-6">
        База данных приложения повреждена. Это может произойти после обновления или некорректного закрытия приложения.
      </p>
...
function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if ((storage as any).isDatabaseCorrupted?.()) {
  ...
  // ============================================================
  // ФУНКЦИИ ДЛЯ ВЫПОЛНЕНИЯ СКРИПТОВ
  // ============================================================
  ...
  // ============================================================
  // ОТПРАВКА ЗАПРОСА (с интеграцией скриптов)
  // ============================================================
    ...
    if (activeTab.collectionId) {
      try {
        ...
        if (collection) {
          ...
          showToast('success', 'Запрос сохранён в коллекцию');
          return;
        }
      } catch (error: any) {
  ...
  const handleSelectRequest = useCallback((collectionId: string, requestId: string) => {
    ...
    if (request) {
      if (activeTab && isTabEmpty(activeTab)) {
        ...
        setTabs(prevTabs => prevTabs.map(tab =>
          tab.id === activeTabId ? { ...tab, savedSnapshot: createSnapshot(request), collectionId } : tab
        ));
      } else {
        if (tabs.length >= MAX_TABS) {
          showToast('error', `Достигнут лимит в ${MAX_TABS} вкладок`);
