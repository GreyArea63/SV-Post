  ...
  try {
    ...
    if (cols) {
      try {
        ...
        await db.transaction(STORES.collections, 'readwrite')
          .objectStore(STORES.collections)
          .put({ id: DEFAULT_ID, data });
      } catch (e) {
    ...
    if (envs) {
      try {
        ...
        await db.transaction(STORES.environments, 'readwrite')
          .objectStore(STORES.environments)
          .put({ id: DEFAULT_ID, data });
      } catch (e) {
    ...
    if (globs) {
      try {
        ...
        await db.transaction(STORES.globals, 'readwrite')
    ...
    if (hist) {
      try {
        ...
        await db.transaction(STORES.history, 'readwrite')
    ...
    if (active) {
      await db.transaction(STORES.activeEnv, 'readwrite')
        .objectStore(STORES.activeEnv)
        .put({ id: DEFAULT_ID, envId: active });
    }
  } catch (error) {
...
/**
 * Получение данных из хранилища с улучшенной обработкой ошибок
 */
