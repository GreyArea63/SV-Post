...
export class ScriptRunner {
  ...
  constructor(context: ScriptContext) {
    ...
    // Работаем с копиями, чтобы не мутировать исходные данные
  ...
  private createPmObject(response?: HttpResponse) {
    ...
    const createExpect = (value: any) => ({
      ...
      // Postman-style: pm.expect(jsonData).to.have.property('id')
      to: {
        have: {
          property: (key: string) => {
            if (typeof value !== 'object' || value === null || !(key in value)) {
    ...
    // Response assertions (pm.response.to.have.status(200))
    ...
    return {
      ...
      // ===== Collection variables (alias for globals in Postman) =====
      collectionVariables: {
        get: (key: string) => self.globals[key] ?? '',
        set: (key: string, value: any) => {
          const strValue = String(value ?? '');
      ...
      // ===== Variables (universal getter/setter) =====
      variables: {
        get: (key: string) => {
          return (
  ...
  /**
   * Выполняет скрипт в изолированной песочнице
   */
  async runScript(
    script: string,
    ...
    try {
      ...
      // Создаём функцию с явными параметрами — блокируем доступ к опасным глобалам
      ...
      await scriptFunction(
        pm,
        pm, // postman — alias для pm
        sandboxConsole,
        undefined, // require заблокирован
        undefined, // process заблокирован
        undefined, // global заблокирован
        undefined, // window заблокирован
        undefined // document заблокирован
      );
